...
router.get("/tutor/suggestions/:lectureId", async (req, res): Promise<void> => {
  ...
  const SYSTEM_PROMPT =
    'You are a rigorous introductory criminal psychology tutor writing study questions. Reply as strict JSON of the form {"questions": string[]} with NO other keys.';
  const buildUserPrompt = (extra: string) =>
    ...
    `THE SINGLE MOST IMPORTANT RULE: every question must contain an explicit, concrete, EVERYDAY example — a specific relatable situation (e.g. "a witness who is completely certain she recognizes the man across the courtroom, yet may be honestly mistaken", "a tired suspect who confesses after hours of questioning just to make it stop", "a parole board trying to decide whether someone is likely to offend again"). Keep examples plain-language and non-technical — no technical terms, study citations, or jargon. The question must ask the student to analyze, explain, judge, or predict something about THAT example. Reuse the lecture's own examples when it has them; otherwise invent a vivid, specific one.\n\n` +
    ...
    `GOOD vs BAD:\n` +
    `- BAD: "What's the difference between a psychopath and an offender?"\n` +
    `- GOOD: "Two children grow up in the same rough neighborhood but only one offends as an adult — what does this suggest about whether a tough environment 'causes' crime?"\n` +
    `- BAD: "What is a false confession?"\n` +
    `- GOOD: "A witness is absolutely certain she has the right man, yet DNA later clears him — explain how an honest, confident witness can still be wrong."\n\n` +
...
router.post("/tutor/ask", async (req, res): Promise<void> => {
  ...
  const sys =
    "You are an encouraging introductory criminal psychology tutor. Explain step by step, use clear examples and relatable cases, and define key terms (e.g. offending, psychopathy, profiling, eyewitness memory, false confession, recidivism) when they come up. Keep replies short (3-6 sentences) unless the student asks for more detail. Never just give the answer — guide them.";
