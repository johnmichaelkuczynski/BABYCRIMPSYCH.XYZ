    ...
    steps.push(
      ...
      await run(`Practice problem ${i + 1}: generate + grade + adapt`, async () => {
        ...
        const gen = await chatJson<{
          ...
          `You generate a single introductory criminal psychology practice problem on "${topic.title}" at easy difficulty, with a short answer (a word, term, "yes"/"no", or short phrase). Respond as strict JSON: {"prompt": string, "correctAnswer": string, "explanation": string}.`,
    ...
    steps.push(
      ...
      await run(
        ...
        async () => {
          ...
          // criminal psychology. Homework is scoped to its own topic's lecture, but
          ...
          const derived = await chatText(
            "You are a strong introductory criminal psychology student answering a short-answer problem. " +
          ...
          const verdict = await chatJson<{
            ...
            "You are an academic quality-control reviewer for a introductory criminal psychology course. " +
...
router.post("/diagnostics/expand-lectures", async (req, res) => {
  ...
  const sys =
    `You are a introductory criminal psychology lecturer producing the ${level.toUpperCase()} version of a lecture. ` +
